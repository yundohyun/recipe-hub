...
public class CaloriesDao {
	...
	public String getCaloriesIdByNameAndCalories(String name, int calories)
		throws SQLException, ClassNotFoundException {
		Connection con = DatabaseUtil.getConnection();
		String sql = "SELECT id FROM calories WHERE name=? AND calories=? ORDER BY created DESC LIMIT 1";
		PreparedStatement st = con.prepareStatement(sql);
		st.setString(1, name);
		st.setInt(2, calories);
